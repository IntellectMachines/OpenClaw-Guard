...
class DecisionEngine {
    ...
    tokenVendor;
    budgetState = new Map();
    constructor(policy, tokenVendor) {
        this.policy = policy;
        this.tokenVendor = tokenVendor || new token_vendor_1.TokenVendingMachine();
    }
    /**
    ...
    async evaluateRequest(req) {
        ...
        try {
            ...
            if (this.policy.denylistedTools?.includes(req.toolName)) {
                return this.createResponse(types_1.GuardDecision.DENY, decisionId, 'Tool is denylisted by policy');
            }
            // Step 0.5: Check allowlist (skip all other checks)
            if (this.policy.allowlistedTools?.includes(req.toolName)) {
                return this.createResponse(types_1.GuardDecision.ALLOW, decisionId, 'Tool is allowlisted');
            }
            // Step 1: Budget Circuit Breaker
            const budgetCheck = this.checkBudget(req);
            if (!budgetCheck.passed) {
                return this.createResponse(types_1.GuardDecision.DENY, decisionId, budgetCheck.reason);
            }
            ...
            if (this.requiresModeration(req.toolName) && req.content) {
                ...
                if (!moderationResult.passed) {
                    return this.createResponse(moderationResult.severity === types_1.ModerationSeverity.CRITICAL
                        ? types_1.GuardDecision.DENY
                        : types_1.GuardDecision.FLAG, decisionId, moderationResult.notes, undefined, moderationResult);
                }
            }
            // Step 3: Authorization Check
            const authCheck = this.checkAuthorization(req);
            if (!authCheck.passed) {
                return this.createResponse(types_1.GuardDecision.DENY, decisionId, authCheck.reason);
            }
            // Step 4: Policy Rules Evaluation
            const ruleResult = this.evaluateRules(req);
            if (ruleResult.matched && ruleResult.decision !== types_1.GuardDecision.ALLOW) {
                return this.createResponse(ruleResult.decision, decisionId, ruleResult.message, undefined, moderationResult);
            }
            // Step 5: Vend JIT Token if needed
            let jitToken;
            if (this.requiresJitToken(req)) {
                try {
                    jitToken = await this.tokenVendor.vendToken(req.targetService || types_1.ServiceType.GENERIC, {
                    ...
                    return this.createResponse(types_1.GuardDecision.DENY, decisionId, `Token vending failed: ${error.message}`);
                }
            }
            // All checks passed - ALLOW
            return this.createResponse(types_1.GuardDecision.ALLOW, decisionId, 'All security checks passed', jitToken, moderationResult);
        }
        catch (error) {
            // Fail closed by default
            return this.createResponse(types_1.GuardDecision.DENY, decisionId, `Evaluation error: ${error.message}`);
        }
    }
    /**
    ...
    checkBudget(req) {
        ...
        if (this.policy.budget.hourlyLimit &&
        ...
        if (this.policy.budget.alertThreshold &&
            usagePercent >= this.policy.budget.alertThreshold) {
            console.warn(`[MOLT-GUARD] Budget alert: User ${userId} has used ${usagePercent.toFixed(1)}% of daily budget`);
        }
    ...
    async moderateContent(content) {
        const issues = [];
        let severity = types_1.ModerationSeverity.NONE;
        let confidence = 1.0;
        ...
        if (this.policy.moderation.detectPii) {
            ...
            for (const [type, pattern] of Object.entries(piiPatterns)) {
                if (pattern.test(content)) {
                    ...
                    severity = types_1.ModerationSeverity.HIGH;
                }
            }
        }
            ...
            for (const term of blockedTerms) {
                if (lowerContent.includes(term.toLowerCase())) {
                    ...
                    if (!allowedTerms.some((a) => lowerContent.includes(a.toLowerCase()))) {
                        ...
                        severity = types_1.ModerationSeverity.CRITICAL;
                    }
                }
            }
        }
        ...
        if (this.policy.moderation.analyzeSentiment) {
            ...
            if (this.policy.moderation.minSentimentScore !== undefined &&
                sentiment < this.policy.moderation.minSentimentScore) {
                issues.push(`Negative sentiment detected (score: ${sentiment.toFixed(2)})`);
                severity =
                    severity === types_1.ModerationSeverity.NONE
                        ? types_1.ModerationSeverity.MEDIUM
                        : severity;
            }
        }
        const passed = severity === types_1.ModerationSeverity.NONE;
        return {
            passed,
            severity,
            categories: issues,
    ...
    checkAuthorization(req) {
        ...
        const userRole = req.metadata.userRole || types_1.UserRole.USER;
        // Tool-specific role requirements
        const roleRequirements = {
            stripe_charge: [types_1.UserRole.ADMIN],
            stripe_refund: [types_1.UserRole.ADMIN],
            aws_deploy: [types_1.UserRole.ADMIN, types_1.UserRole.SERVICE],
            delete_user: [types_1.UserRole.ADMIN],
            modify_permissions: [types_1.UserRole.ADMIN],
        };
    ...
    evaluateRules(req) {
        ...
        for (const rule of sortedRules) {
            ...
            if (rule.targetTools.length > 0 &&
                !rule.targetTools.includes(req.toolName)) {
                continue;
            }
            // Check all conditions (AND logic)
            const allConditionsMet = rule.conditions.every((condition) => this.evaluateCondition(condition, req));
            if (allConditionsMet) {
                return {
                    matched: true,
        ...
        return {
            matched: false,
            decision: types_1.GuardDecision.ALLOW,
    ...
    evaluateCondition(condition, req) {
        ...
        switch (condition.operator) {
            case types_1.ConditionOperator.EQUALS:
                return value === condition.value;
            case types_1.ConditionOperator.NOT_EQUALS:
                return value !== condition.value;
            case types_1.ConditionOperator.CONTAINS:
                return String(value).includes(String(condition.value));
            case types_1.ConditionOperator.NOT_CONTAINS:
                return !String(value).includes(String(condition.value));
            case types_1.ConditionOperator.GREATER_THAN:
                return Number(value) > Number(condition.value);
            case types_1.ConditionOperator.LESS_THAN:
                return Number(value) < Number(condition.value);
            case types_1.ConditionOperator.MATCHES_REGEX:
                return new RegExp(String(condition.value)).test(String(value));
            case types_1.ConditionOperator.IN_LIST:
                return Array.isArray(condition.value) && condition.value.includes(value);
            case types_1.ConditionOperator.NOT_IN_LIST:
                return Array.isArray(condition.value) && !condition.value.includes(value);
            default:
                return false;
        }
     ...
     */
    createResponse(decision, decisionId, moderationNotes, jitToken, moderationResult) {
        return {
            decision,
            jitToken,
            moderationNotes,
            ...
            decisionId,
            timestamp: Date.now(),
            cacheTtl: decision === types_1.GuardDecision.ALLOW ? 60 : 0,
        };
    }
    /**
 ...
 */
function createDecisionEngine(policyOverrides) {
    const defaultPolicy = {
        version: '1.0.0',
        name: 'default-policy',
